...
export default {
  ...
  methods: {
    handleSelectCode(item) {
      ...
      if (item.selected && !this.sorted.includes(item.code)) {
        this.sorted.push(item.code)
      }

      if (!item.selected && this.sorted.includes(item.code)) {
        this.sorted.map((val, i) => {
          if (val === item.code) {
      ...
      if (this.sorted.length > 7) {
        const first = this.sorted.shift()
        this.list.map(m => {
          if (m.code === first) {
            m.selected = false
          }
...
.headerFIVE {
  padding: 0 0.625rem;
  background: url(../../assets/images/hash_zuhe_head.png) no-repeat;
